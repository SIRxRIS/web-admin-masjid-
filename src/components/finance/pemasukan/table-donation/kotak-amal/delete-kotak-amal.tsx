...
import { deleteKotakAmalAction } from "@/actions/kotak-amal";
...
export function DeleteKotakAmalDialog({
  ...
  const handleConfirm = async () => {
    try {
      const result = await deleteKotakAmalAction(kotakAmalId);

      if (!result.success) {
        console.error("Error deleting kotak amal:", result.error);
        Swal.fire({
          title: "Error!",
          text: result.error || "Gagal menghapus data kotak amal",
          icon: "error",
          timer: 2000,
          timerProgressBar: true,
          showConfirmButton: false,
