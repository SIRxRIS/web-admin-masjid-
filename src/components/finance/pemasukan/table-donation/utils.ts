...
// Import fungsi dari lib/utils.ts
import { formatAngkaInput, parseFormattedNumber } from "@/lib/utils";

// Re-export untuk kompatibilitas
export { formatAngkaInput, parseFormattedNumber };

// Menggunakan fungsi yang lebih baik dari lib/utils.ts
export const formatNumber = (value: string): string => {
  return formatAngkaInput(value);
};
...
// Fungsi tambahan untuk konversi ke number - diperbaiki
export const parseToNumber = (value: string): number => {
  return Number(parseFormattedNumber(value));
};
