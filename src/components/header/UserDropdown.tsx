...
export default function UserDropdown() {
    ...
    return (
      <div className="relative">
        <div className="flex items-center text-gray-700 dark:text-gray-400">
          <div className="mr-3 overflow-hidden rounded-full h-11 w-11 bg-gray-200 dark:bg-gray-700 animate-pulse">
            <div className="w-full h-full bg-gray-300 dark:bg-gray-600"></div>
          </div>
          <div className="block mr-1 font-medium text-theme-sm">
  ...
  // Not authenticated state: tampilkan tombol Masuk hanya jika tidak ada user
  if (!user) {
    return (
      <div className="relative">
        <Link
          href="/signin"
          className="flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200"
        >
          Masuk
        </Link>
      </div>
  ...
  // Safely derive name values even when userProfile is null
  const firstName = userProfile?.first_name?.trim() || '';
  const lastName = userProfile?.last_name?.trim() || '';
  const combinedName = `${firstName} ${lastName}`.trim();

  // Get display name - prioritizes full_name, then first+last, then email, then fallback
  const displayName =
    (userProfile?.full_name?.trim() || combinedName || user?.email || 'User');

  // Get short name for button (first name, first part of full_name, or email prefix)
  const shortName =
    firstName ||
  ...
  return (
    <div className="relative">
      {showExitOverlay && (
        <div
          aria-hidden
          className="fixed inset-0 z-[9999] bg-black/40 backdrop-blur-[2px] transition-opacity duration-400 opacity-100"
      ...
      >
        <div>
          ...
          {(userProfile as any)?.jabatan && (
            <span className="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400 italic">
              {(userProfile as any)?.jabatan}
            </span>
          )}
        </div>
